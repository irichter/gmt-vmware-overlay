...
pkg_setup() {
	if kernel_is ge 3 19 0; then
		if use vmci || use vsock; then
			eerror "For Kernel 3.19, vmci and vsock from this package does not build."
			eerror "Use the in-kernel alternatives and disable the useflags on this package:"
			eerror "VMWARE_VMCI and VMWARE_VMCI_VSOCKETS"
			die "Incompatible useflags for your kernel"
		fi
	fi
...
src_prepare() {
	...
	kernel_is ge 3 19 0 && epatch "${FILESDIR}/${PV_MAJOR}-3.19.0.patch"
...
src_install() {
	...
	local mpdfile="${ED}/etc/modprobe.d/vmware.conf"
	echo '# VMWare modules aliases' > "${mpdfile}"
	use vmci || echo 'alias vmci vmw_vmci' >> "${mpdfile}"
	use vsock || echo 'alias vsock_vmw vmw_vsock_vmci_transport' >> "${mpdfile}"
	use vsock && echo 'alias vsock_vmw vsock' >> "${mpdfile}"
}
